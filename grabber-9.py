...
try:
    ...
    count = max([int(file.split('.')[0]) + 1 for file in os.listdir(f'{directory}/dataset/unparsed_json')], default=0)
    for _ in range(100000):
        ...
        print(f'Before Refresh: {len(driver.requests)}')
        del driver.requests
        # driver.delete_all_cookies()
        # driver.refresh()
        print(f'After Refresh: {len(driver.requests)}')
